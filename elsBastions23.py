...
#Add a column with average pace
df['average_pace'] = round(60/df['average_speed'],2)

# Filter rows from the current year
current_year = datetime.now().year
df_2023 = df[df['date'].dt.year == current_year]


start_date = pd.to_datetime('2023-03-01')
end_date = pd.to_datetime('2023-05-12')
bastions_df = df_2023[(df_2023['date'] >= start_date) & (df_2023['date'] <= end_date)]
print(bastions_df)
#print(bastions_df.columns)

stats_columns = ['moving_time','distance','elevation_gain','average_pace']
stats_df = bastions_df[stats_columns]
...
df_moving_time = bastions_df.groupby(['date'])['moving_time'].agg('sum').reset_index()
...
plt.title('Últims 2 mesos pre-Bastions')
# Display the plot
plt.show()
...
df_elevation = bastions_df.groupby(['date'])['elevation_gain'].agg('sum').reset_index()
...
bastions_df.sort_values('date', inplace=True)

# Calculate the rest days between consecutive training sessions
bastions_df['rest_days'] = bastions_df['date'].diff().dt.days - 1

# Calculate the average rest days
average_rest_days = bastions_df['rest_days'].mean()
...
bastions_df['distance_interval'] = pd.cut(bastions_df['distance'], bins=bins, right=False)

# Count the number of runs in each distance interval
runs_per_interval = bastions_df['distance_interval'].value_counts().sort_index()
...
# Assign each run to a distance interval
bastions_df['elevation_interval'] = pd.cut(bastions_df['elevation_gain'], bins=bins, right=False)

# Count the number of runs in each distance interval
runs_per_interval = bastions_df['elevation_interval'].value_counts().sort_index()
...
# Assign each run to a distance interval
bastions_df['duration_interval'] = pd.cut(bastions_df['moving_time'], bins=bins, right=False)

# Count the number of runs in each distance interval
runs_per_interval = bastions_df['duration_interval'].value_counts().sort_index()
...
plt.ylabel('Frequency')
plt.title('Number of Runs vs. Duration Intervals Bastions')
...
plt.scatter(bastions_df['distance'], bastions_df['elevation_gain'])
...
weekly_data = bastions_df.groupby(pd.Grouper(key='date', freq='W-MON')) \
               .agg({'moving_time': 'sum', 'distance': 'sum', 'elevation_gain': 'sum'}) \
               .reset_index()

fig, ax = plt.subplots()
ax.bar(weekly_data.index, weekly_data['moving_time'], label='Duration (min)',color='lightblue')
ax.set_xticks(weekly_data.index)
ax.set_xticklabels(weekly_data['date'].dt.strftime('%m-%d'))
...
fig, ax = plt.subplots()
ax.bar(weekly_data.index, weekly_data['elevation_gain'], label='Elevation Gain (m)',color='lightblue')
ax.set_xticks(weekly_data.index)
ax.set_xticklabels(weekly_data['date'].dt.strftime('%m-%d'))
plt.xticks(rotation=60)
ax.legend()
...
bastions_df['week_number'] = bastions_df['date'].dt.week

# Group the DataFrame by week number and count the number of unique dates
training_days_per_week = bastions_df.groupby('week_number')['date'].nunique()

# Calculate the number of rest days per week
rest_days_per_week = 7 - training_days_per_week

print("Rest days per week:")
print(rest_days_per_week)

# Create a new column for the week number
bastions_df['week_number'] = bastions_df['date'].dt.week

# Group the DataFrame by week number and count the number of training sessions
trainings_per_week = bastions_df.groupby('week_number').size()
...
sorted_df = bastions_df.sort_values('elevation_gain',ascending='False')
...
# Assuming you have two DataFrames named df1 and df2 containing your training logs

# Plotting the first training period
plt.figure(figsize=(8, 5))  # Set the desired width and height of the figure
plt.scatter(bastions_df['distance'], bastions_df['elevation_gain'], label='els Bastions')

# Plotting the second training period
plt.scatter(montseny_df['distance'], montseny_df['elevation_gain'], label='Montseny')

# Adding labels and title to the plot
plt.xlabel('Distance')
plt.ylabel('Elevation Gain')
plt.title("Distribució per distancia i desnivell positiu")
